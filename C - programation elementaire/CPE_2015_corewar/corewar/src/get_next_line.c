...
** Last update Sun Mar 27 21:00:58 2016 Hugo Willaume
*/
...
char		*my_strcot(char *str1, char *str2, int i, int tmp)
{
  char		*str3;
...
char		*get_next_line(const int fd)
{
  ...
  if ((str = malloc(((tmp = READ_SIZE) + 1) * sizeof(char))) == NULL
      || (i = read(fd, str, READ_SIZE)) == -1 || ((i == 0) && save == NULL))
    return (NULL);
  str[i] = 0;
  i = 0;
  str = (save != NULL) ? my_strcot(save, str, 0, 0) : str;
  while (str[i + 1] != 0)
